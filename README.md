...
个人知识管理的 Markdown 笔记应用。用笔记本与标签组织笔记，支持全文搜索；编辑器分栏实时预览，可对文字自选颜色；支持自定义壁纸背景与键入音效。
...
```
{{#ef4444}}红色文字{{/}}  这是一段 {{#3b82f6}}蓝色{{/}} 文字
```

`{{#hex}}` 表示开始着色，`{{/}}` 表示结束；`hex` 为十六进制颜色（可不带 `#`）。同一行内可用不同颜色、不同区间各自着色，互不影响。着色区间内的文字按纯文本处理，不解析嵌套的 Markdown 格式。
